...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        ...
        <div id="kt_content_container" class="container-xxl">
            ...
            <div class="card card-flush">
                ...
                <div class="card-header mt-5">
                    ...
                    <div class="card-title">
                        <form class="d-flex align-items-center position-relative my-1" action="{{ route('location.territories') }}" method="GET">
                            <!--begin::Is Active-->
                            <div class="w-200 mw-250px me-2">
                                ...
                                <select name="is_active" class="form-select form-select-solid" data-control="select2" data-placeholder="Search Is Active" data-kt-ecommerce-product-filter="status" data-hide-search="true">>
                                    ...
                                    @foreach ($is_actives as $is_active)
                                        <option value="{{ $is_active }}" {{ request('is_active') == $is_active ? 'selected' : '' }}>
                                            {{ $is_active }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="card-body pt-0">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                        ...
                        <tbody class="text-black-600">
                            @foreach ($territories as $key => $territory)
                                <tr>
                                    <!--begin::S.N=-->
                                    <td>{{ $territories->firstItem() + $key }}</td>
                                    <!--end::S.N=-->
                                    ...
                                    <td>
                                        {{ $territory->name }}
                                        <br>
                                        <span style="color: blueviolet">Associated Sub Area, Area, Region, Zone: </span>
                                        <br>
                                    ...
                                    <!--begin::Created Date=-->
                                    <td>{{ $territory->created_at->setTimezone('Asia/Dhaka')->format('d M, Y') }}</td>
                                    <!--end::Created Date=-->

                                    <!--begin::Updated Date=-->
                                    <td>{{ $territory->updated_at->setTimezone('Asia/Dhaka')->format('d M, Y') }}</td>
                                    <!--end::Updated Date=-->

                                    <!--begin::Is Active=-->
                                    <td>
                                        <select
                                            data-current_status="{{ $territory->is_active }}"
                                            onchange="territory(this, {{ $territory->id }})"
                                            class="form-select form-select-solid status" 
                                            data-control="select2" 
                                            data-hide-search="true">
                                            @foreach ($is_actives as $is_active)
                                                <option value="{{ $is_active }}" {{ $territory->is_active == $is_active ? 'selected' : '' }}>
                                                    {{ $is_active }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="pagination-container mt-10">
                        <nav>
                            {{ $territories->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-5') }}
                        </nav>
                    </div>
                </div>
                <!--end::Card body-->
            ...
            <!--begin::Modals-->
            <!--begin::Modal - Add Territory-->
            @include('Market::territories.modals.add')
            <!--end::Modal - Add Territory-->

            <div class="modal fade" id="deactivateTerritoryModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border border-warning">
                        ...
                        <div class="modal-body bg-light-warning">
                            <p>Are you sure you want to deactivate this territory?</p>
                        </div>
...
@push('scripts')
    <script>
        function territory(selectElement, territoryId) {
            const $select = $(selectElement);
            const newStatus = $select.val();

            if (newStatus === 'Inactive') {
                // Show the modal
                $('#deactivateTerritoryModal').modal('show');

                // Handle the confirm deactivation
                $('#confirmDeactivation').off('click').on('click', function () {
                    proceedWithAreaUpdate();
                    $('#deactivateTerritoryModal').modal('hide');
                });
            ...
            function proceedWithAreaUpdate () {
                let url = "{{ route('location.update_territory', ['id' => ':id']) }}";
                let action = url.replace(':id', territoryId);
