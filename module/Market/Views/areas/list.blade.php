...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        ...
        <div id="kt_content_container" class="container-xxl">
            ...
            <div class="card card-flush">
                ...
                <div class="card-header mt-5">
                    ...
                    <div class="card-title">
                        <form class="d-flex align-items-center position-relative my-1" action="{{ route('location.areas') }}" method="GET">
                            <!--begin::Is Active-->
                ...
                <div class="card-body pt-0">
                            ...
                            @foreach ($areas as $key => $area)
                                <tr>
                                    ...
                                    <td>
                                        {{ $area->name }}
                                        <br>
                                        <span style="color: blueviolet">Associated Region, Zone: </span>
                                        <br>
                                        {{ $area->region->name }}
                                        /
                                    ...
                                    <!--begin::Created Date=-->
                                    <td>{{ $area->created_at->setTimezone('Asia/Dhaka')->format('d M, Y') }}</td>
                                    <!--end::Created Date=-->

                                    <!--begin::Updated Date=-->
                                    <td>{{ $area->updated_at->setTimezone('Asia/Dhaka')->format('d M, Y') }}</td>
                                    <!--end::Updated Date=-->

                                    <!--begin::Is Active=-->
                                    <td>
                                        <select
                                            data-current_status="{{ $area->is_active }}"
                                            onchange="area(this, {{ $area->id }})"
                                            class="form-select form-select-solid zone_status" 
                                            data-control="select2" 
                                            data-hide-search="true">
                                            @foreach ($is_actives as $is_active)
                                                <option value="{{ $is_active }}" {{ $area->is_active == $is_active ? 'selected' : '' }}>
                                                    {{ $is_active }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="pagination-container mt-10">
                        <nav>
                            {{ $areas->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-5') }}
                        </nav>
                    </div>
                </div>
                <!--end::Card body-->
            ...
            @include('Market::areas.modals.add')
            <!--end::Modal - Add permissions-->

            <div class="modal fade" id="deactivateAreaModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border border-warning">
                        ...
                        <div class="modal-body bg-light-warning">
                            <p>Are you sure you want to deactivate this area?</p>
                        </div>
    ...
    <script>
        function area(selectElement, areaId) {
            ...
            function proceedWithAreaUpdate () {
                let url = "{{ route('location.update_area', ['id' => ':id']) }}";
                let action = url.replace(':id', areaId);
