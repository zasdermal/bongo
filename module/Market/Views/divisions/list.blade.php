...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        ...
        <div id="kt_content_container" class="container-xxl">
            ...
            <div class="card card-flush">
                ...
                <div class="card-header mt-5">
                    ...
                    <div class="card-title">
                        <form class="d-flex align-items-center position-relative my-1" action="{{ route('location.divisions') }}" method="GET">
                            <!--begin::Is Active-->
                ...
                <div class="card-body pt-0">
                        ...
                        <tbody class="text-black-600">
                            @foreach ($divisions as $key => $division)
                                <tr>
                                    ...
                                    <td>
                                        @if ($division->user)
                                            {{ $division->user->employee->name }} - ({{ $division->user->username }})
                                        @else
                                            Not Assigned
                                        @endif
                                    </td>
                                    ...
                                    <!--begin::Created Date=-->
                                    <td>{{ $division->created_at->setTimezone('Asia/Dhaka')->format('d M, Y') }}</td>
                                    <!--end::Created Date=-->

                                    <!--begin::Updated Date=-->
                                    <td>{{ $division->updated_at->setTimezone('Asia/Dhaka')->format('d M, Y') }}</td>
                                    <!--end::Updated Date=-->

                                    <!--begin::Is Active=-->
                                    <td>
                                        <select
                                            data-current_status="{{ $division->is_active }}"
                                            onchange="sub_area(this, {{ $division->id }})"
                                            class="form-select form-select-solid status" 
                                            data-control="select2" 
                                            data-hide-search="true">
                                            @foreach ($is_actives as $is_active)
                                                <option value="{{ $is_active }}" {{ $division->is_active == $is_active ? 'selected' : '' }}>
                                                    {{ $is_active }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="pagination-container mt-10">
                        <nav>
                            {{ $divisions->appends(request()->query())->onEachSide(1)->links('pagination::bootstrap-5') }}
                        </nav>
                    </div>
                </div>
                <!--end::Card body-->
            ...
            <!--begin::Modals-->
            <!--begin::Modal - Add Sub Area-->
            @include('Market::divisions.modals.add')
            <!--end::Modal - Add Sub Area-->

            <div class="modal fade" id="deactivateSubAreaModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border border-warning">
                        ...
                        <div class="modal-body bg-light-warning">
                            <p>Are you sure you want to deactivate this sub area?</p>
                        </div>
    ...
    <script>
        function sub_area(selectElement, subAreaId) {
            ...
            function proceedWithAreaUpdate () {
                let url = "{{ route('location.update_sub_area', ['id' => ':id']) }}";
                let action = url.replace(':id', subAreaId);
